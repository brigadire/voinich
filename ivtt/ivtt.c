...
/*
   Intermediate Voynich Transliteration Tool.
   Processes files in the IVTFF format.
   This is version 2.4 of 07/09/2025.
*/
...
int kcomt=0;     /* Keep textual comment lines, both hash and inline */
int hash=0;      /* Special hash comment setting due to locus selection */
int kcomg=0;     /* Keep <-> sign (previously: - ) and <~> sign */
int kcomp=0;     /* Keep <$> sign (previously: = ) and <%> sign */
int kcomv=0;     /* Keep page-variable related information <! and <@ */
int s_hspc=0;    /* Do not touch dot spaces */
int s_uspc=0;    /* Do not touch comma spaces */
...
int toppar=0;    /* Keep all P locus text lines */
...
int noof=0;      /* Normal output to output file */
...
int nldrpi = 0   /* Number of lines with !-locator dropped */;
...
int comlin=0        /* 1 if line starts with # - set in GetLine */;
int filehead=0      /* ==2 if line 1 and a valid header - set in GetLine*/;
int hastrtxt=0      /* 1 if there is transliterated text - set in GetLine */;
int hasfoli=0       /* 1 if there are < > starting in the first position */;
int newpage=0       /* 1 if there are < > without a period (i.e. a new page) */;
int nwpar=0         /* 1 if the line includes a <%> code */;
...
char txtag[27]               /* List of text tag settings */;
int hastag          /* 1 if a page header indicates the use of text tags */;
...
int tagcom = 0;   /* Not 0 if this is a 6-char text tag comment */
char comchr = ' '; /* The character defining the type of inl.comment */
int indtag = 0;   /* Text tag index */
...
/* Reset all page variables and text tags */
...
int usepgloc(int iopt)

/* This used to be called 'usepage'
   If called with iopt == 1 (once per page)
   check the list of pgvar[] (only) with include and
   exclude options given by the user.

   If called with iopt == 0 (once per locus), do three things:

   (1) Check the list of pgvar[] and txtag[] with include and
   exclude options given by the user.
   (2) Then use page var. <at> to select 
   loci, either as single or dual-character locus types.
   Also check for paragraph start lines if the locus is P
   (3) Check if this is an invalid locus (locator == '1')

   The value of pgvar[0] was set in GetLine.
   The values of txtag[] were set in Track (invoked by PrepLine)
*/

{
  ...
  /* fprintf(stderr,"usepgloc called, option %d\n",iopt); */
  if (iopt) {
  
    /* Here it is called for the whole page. Only look at the page
       variables. If a variable is set to @, ignore it  */
    hastag = 0;
    for (i=1; i<=26; i++) {
      usevar = pgvar[i];
      if (invar[i] != ' ') {
    ...
    */ 
    ...
    /* Here it is called for a locus (or the file header) */
    if (filehead) return 1;

    /* First check the combination of page variables and text tags. */
    for (i=1; i<=26; i++) {
      /* decide whether to go by page var. or by text tag */
      usevar = pgvar[i];
      if (usevar == '@') {
        usevar = txtag[i];
        /* fprintf(stderr,"Checking text tag %d : %c\n",i,usevar); */
      } else {
        if (usevar != ' ') {
          /* fprintf(stderr,"Checking page var %d : %c\n",i,usevar); */
        }
      }
      if (usevar == '@') usevar = ' ';
      ...
      if (invar[i] != ' ') {
        if (invar[i] != usevar) select=0;
        /* fprintf(stderr,"Incl.ind: %d , val: %c , result: %d\n",i,invar[i],select); */
      }
      if (exvar[i] != ' ') {
        if (exvar[i] == usevar) select=0;
      }
    ...
    /* Only if it is selected, then also check the 
       locus type, 1 or 2 char */
    if (select) {
      if (invar[0] != ' ') {
        mat = (invar[0] != loc2[0]);
        if (uloc2) {
    ...
    /* Handle processing related to -q option */
    if (select && (toppar > 0)) {
      if (loc2[0] == 'P') {
        if (toppar == 1 && nwpar == 0) select = 0;
        if (toppar == 2 && nwpar) select = 0;
      }
    }

    /* Only if it is selected, then check the locator code */
    if (select) {
      if (cator == '!') {
        select=0;
        nldrpi += 1;
...

{
    ...
    tagcom = 0;    /* >0 if a text tag comment */
  ...
  int index;*/
{
  ...
  if (cb == '>') {
    if (in_comm > 0) {
      if (dedcom) {
        ...
        dedcom = 0;
        /* Check for first line of paragraph */
        if (comchr == '%') {
          nwpar = 1;
        }
      }
  ...
  /* New < > comment, decode its type */
  if (in_comm == 1) {
    comchr = cb;
    tagcom = (comchr == '@');
    if (tagcom == 0) dedcom = (comchr != '!');

    /* if (dedcom) fprintf(stderr,"%s\n","Dedicated commment found");
       if (tagcom) fprintf(stderr,"%s\n","Text tag found"); */
  }
  ...
  if (tagcom) {
    if (in_comm == 2) {
      ...
      /* fprintf(stderr,"Found text tag index %d\n",indtag); */
    }
    if (in_comm == 4) {
      if (indtag) txtag[indtag] = cb;
      /* fprintf(stderr,"Found text tag value %c\n",cb); */
    }
  }
...
/* Parse command line options. There can be many and each should be
   ...
   -pv with lower case p: one of:
     a,b,c,f,h,l,m,p,q,s,u,w,x where v can be 0-9, plus optionally -tC where 
  ...
  char *argv[];*/
{
      ...
      if (optn >= '@' && optn <= 'Z') {
        ...
        switch (optn) {
           case 'a':
             ...
             break;
           case 'b':
             /* this no longer looks at now obsolete % and ! placeholders */
             if (val == '0') {
               white = 0;               /* Keep blanks */
             }
             else if (val == '1') {
               white = 1;               /* Strip blanks */
             }
             break;
           case 'c':
             if (val == '4')  val='1';                   /* Backward compatibility */
             if (val == '0')      { kcomt=0; kcomv=0; }  /* Keep t and v comments */
             else if (val == '1') { kcomt=1; kcomv=0; }  /* Strip t cmts # and <! > */
             else if (val == '2') { kcomt=0; kcomv=1; }  /* Strip v cmts  */
             else if (val == '3') { kcomt=1; kcomv=1; }  /* Strip all */
             hash = kcomt;
             break;
           case 'f':
             if (val == '0')        kfoli=0;             /* Keep foliation info */
             else if (val == '1') { kfoli=1; kcomv=1; }  /* Strip it and remove v comments */
             else if (val == '9') folign=1; /* Ignore locus layout */
                          /* but still process page headers */
             break;
           case 'm':
             if (val == '0')      { mute=0; noof=0; }  /* All output to stderr & o-f */
             else if (val == '1') { mute=1; noof=0; }  /* suppress warnings */
             else if (val == '2') { mute=2; noof=0; }  /* suppress all stderr */
             else if (val == '3') { mute=1; noof=1; }  /* suppress warnings & no o-f */
             /* The setting of this option is not reported */
             break;
           case 'p':
             if (val == '0') {
               kcomg=0; kcomp=0; /* Keep <-> and <~>, <%> and <$> */
             } else if (val == '1') {
               kcomg=1; kcomp=0; /* <-> , <~> to space */
             } else if (val == '2') {
               kcomg=1; kcomp=1; /* <-> , <~> to space and <%> , <$> stripped */
             } else if (val == '3') {
               kcomg=2; kcomp=1; /* <-> , <~> , <%> and <$> stripped */
             }
             break;
           case 'q':
             if (val == '0') {
               toppar=0; /* Keep all P locus lines */
             } else if (val == '1') {
               toppar=1; /* Keep only top P locus lines */
               kcomp=1; /* and remove paragraph markers */
             } else if (val == '2') {
               toppar=2; /* Delete all top P locus lines */
               kcomp=1; /* and remove paragraph markers */
             }
             break;
           case 'l':
             ...
             else if (val == '2') liga=2; /* Change to capitalisation rule */
             else if (val == '3') liga=3; /* As 3, but change cth to CTh.. */
             break;
           case 'h':
             if (val == '0') s_uspc = 0;      /* Keep comma for uncertain space*/
             else if (val == '1') s_uspc = 1; /* Treat comma as dot */
             else if (val == '2') s_uspc = 2; /* Strip uncertain spaces */
             else if (val == '3') {
               s_uspc = 3;                    /* Turn uncertain spaces to ? */
               unr = 1;                       /* And turn words with ? into ??? */
             }
             break;
           case 's':
             if (val == '0') {
               s_hspc = 0; cue = '.';      /* Keep dot for hard space*/
             } else if (val == '1') {
               s_hspc = 1; cue = ' ';      /* Turn hard space to blank */
             } else if (val == '2') {
               s_hspc = 2; cue = (char) 0; /* Strip hard space */
               s_uspc = 2;        /* And also uncertain spaces of course */
             } else if (val == '3') {
               s_hspc = 3; cue = '.';     /* Convert space to newline */
               kfoli = 1;         /* For this also remove foliation */
             }
             break;
           case 't':
             auth = val;  /* This also used to strip placeholders */
           ...
           case 'x':
             if (val == '0') {
               kcomt = 1; kcomv = 0; kcomg = 1; kcomp = 1;
               wrap = 1; white = 1; 
             } else if (val == '1') {
               kcomt = 1; kcomv = 1; kfoli = 1; kcomg = 1;
               kcomp = 1; white = 1; 
             } else if (val == '2') {
               kcomt = 1; kcomv = 1; kfoli = 1; kcomg = 1;
               kcomp = 1; white = 1; brack = 1;
             } else if (val == '3') {
               s_hspc = 1; s_uspc = 1; cue = ' ';
             } else if (val == '4') {
               s_hspc = 1; s_uspc = 2; cue = ' ';
             } else if (val == '5') {
               liga = 1; a_high = 2;
             } else if (val == '6') {
               liga = 3; a_high = 2;
             } else if (val == '7') {
               kcomt = 1; kcomv = 1; kfoli = 1; brack = 1; liga = 1; 
               white = 1; kcomg = 1; kcomp = 1;
               s_hspc = 1; cue = ' '; s_uspc = 1;
             } else if (val == '8') {
               kcomt = 1; kcomv = 1; kfoli = 1; brack = 1; liga = 1; 
               white = 1; kcomg = 1; kcomp = 1;
               s_hspc = 1; cue = ' '; s_uspc = 2;
             }
             hash = kcomt;
             break;
        }
  ...
  if (s_hspc == 3) {
    /* If spaces converted to line breaks: no line wrapping allowed */
    wrap = 1;
  }
  if (s_hspc >= 2) {
    /*  If spaces deleted or converted to NL: <-> , <~> treated the same */
    kcomg = 2;
  }
  return 0;
}
...
char *argv[];*/
  ...
  if (mute == 0) {
    ...
    /* Process each one, but more in order of importance */
    switch (kfoli) {
       case 0: fprintf (stderr,"%s\n","Keep foliation");
           break;
       case 1: fprintf (stderr,"%s\n","Remove foliation and file header");
           break;
    }
    switch (folign) {
    ...
    switch (kcomt) {
       case 0: fprintf (stderr,"%s\n","Keep textual comments");
           break;
       case 1: fprintf (stderr,"%s\n","Remove textual comments");
           break;
    }
    switch (kcomv) {
       case 0: fprintf (stderr,"%s\n","Keep page variable and text tag comments");
           break;
       case 1: fprintf (stderr,"%s\n","Remove page variable and text tag comments");
           break;
    }
    switch (a_high) {
    ...
    switch (s_uspc) {
       case 0: fprintf (stderr,"%s\n","Use comma for uncertain spaces");
           break;
       case 1: fprintf (stderr,"%s\n","Treat uncertain spaces as normal spaces (as above)");
           break;
    ...
    switch (kcomp) {
       case 0: fprintf (stderr,"%s\n","Keep paragraph start / end code");
           break;
       case 1: fprintf (stderr,"%s\n","Remove paragraph start / end code");
           break;
    }
    switch (kcomg) {
       case 0: fprintf (stderr,"%s\n","Keep drawing intrusion code");
           break;
       case 1: fprintf (stderr,"%s\n","Change drawing intrusion code into space");
           break;
    ...
    if (invar[0] != ' ') {
      ...
      nlcopt = 1;
      if (kcomt == 0) {
        fprintf (stderr,"This suppresses hash comment lines\n");
        hash = 1;
      }
    }
    if (exvar[0] != ' ') {
      if (uloc2) {
        fprintf (stderr,"Exclude locus type %c%c\n", exvar[0], uloc2);
      } else {
    ...
    /* In this version disable overruling paragraph line handling by locus selection */
    /* if (invar[0] != ' ' && invar[0] != 'P') toppar = -1; */
    /* if (exvar[0] == 'P') toppar = -1; */
    switch (toppar) {
       case -1: fprintf (stderr,"%s\n","Paragraph text line selection disabled");
           break;
       case 0: fprintf (stderr,"%s\n","Keep all paragraph text lines");
           break;
       case 1: fprintf (stderr,"%s\n","Delete all but paragraph top text lines");
           break;
       case 2: fprintf (stderr,"%s\n","Delete paragraph top text lines");
           break;
    }
...
/*char *buf;*/
{
  ...
  /* Now initialise filehead based on the first line of the file.
     Value 3 if a VDBTF header is found (error).
     Value 2 if it is a complete IVTFF header.
     Value 1 if it is a comment, or 0 if it is neither. */

  if (nlread == 0) {
    filehead = 3;
    for (ii=1; ii<=7; ii++) {
      if (buf[ii] != dtest[ii]) filehead = 1;
    }
    if (filehead == 3) {
      if (mute < 2) {
        fprintf (stderr, "Input file has VDBTF header - not supported.\n");        
      }
      return 1;
    }
    filehead = 2;
    for (ii=1; ii<=7; ii++) {
      if (buf[ii] != ctest[ii]) filehead = 1;
    }
    if (comlin == 0) filehead = 0;
    if (filehead != 2) {
      if (mute == 0) fprintf (stderr, "Input file has no IVTFF header.\n");        
    }
    /* Check if the file uses STA */
    if (filehead ==2) {
      if ((buf[8]=='I') && (buf[9]=='S')) {
        if ((unr != 0) || (brack > 1)) {
          if (mute < 2) {
            fprintf (stderr, "Input file uses the STA alphabet,\n");        
            fprintf (stderr, "options related to ? will not work.\n");        
          }
          return 1;
        }
      }
...
/*char *buf1, *buf2;*/
{
  ...
  /* No transliterated text on page header, only needed if foliation removed */
...
/*char *buf;*/
{
  ...
  if (brack == 0 && unr == 0 && liga < 2) return ret;
  ...
  while (cb = buf[index]) {
    ...
    if (ind_altc >= 0) {
      if (brack == 1) {
        ...
        /* Process into unreadable or
           prepare for processing into ??? */
        if (unr == 0) {
          index = ind_alto;
        } else if (unr == 1) {
          index = ind_altc;
      ...
      /* Shift left remainder of line if needed */
      if (index < ind_altc) {
...
/* Process spaces in buffer. This treats occurrences of
   comma and dot (but not % !) in input text, but also
   dedicated comments related to drawing intrusions and end
   of paragraph.
   In addition, it removes text tags if necessary.
...
/*  if (in_comm == -1) fprintf(stderr,"%s","Start of comment\n");
    ...
    if (comlin == 0 && in_comm == 0 && in_foli == 0) {
      ...
      cbo = cb;
      if (cb == ',') {
        if (s_uspc == 1) cb = cbo = '.';  /* Treat same as dot space*/
        else if (s_uspc == 2) addchar=0;  /* Convert to blank */
        else if (s_uspc == 3) cbo = '?';  /* Convert to unreadable */
      }
      if (cb == '.') {
        if (s_hspc == 1) cbo = ' ';       /* Just use blank instead */
        else if (s_hspc == 2) addchar=0;  /* Convert to nothing */
        else if (s_hspc == 3) cbo = '\n';  /* Convert to line break */
      }
      cb = cbo;
...
/* Write buffer to output, optionally skipping foliation info,
   ...
   If foliation is suppressed, blank spaces before transliterated text
...
/*char *buf;*/
{
  ...
  if (comlin != 0 && filehead != 2) {
    /*  Print immediately or exit  */
    if (hash == 1) {
      nlhash +=1;
    } else {
      /* do not output if there is a pending header */
      if (pend_hd) {
        nlhash += 1;
      } else {
        OutString(buf);
  ...
  if (pend_hd) {
    /* fprintf(stderr, "%s\n", "Pending header output (if allowed)"); */
    pend_hd = 0;
...
char *argv[];*/
{
  ...
  int selpage = 0, selloc = 0;
  int erropt;

  /* For reference: */
  char *what = "@(#)ivtt\t\t2.4\t2025/09/07 RZ\n";

  /* Parse command line options */
  /* Do this first, in order to get the "mute" option first */
  erropt = ParseOpts(argc, argv);
  if (mute == 0) {
    fprintf (stderr,"Intermediate Voynich Transliteration Tool (v 2.4)\n\n");
  }
  ...
  /* If in diagnostics mode, say so */
  if (noof) {
    fprintf (stderr, "\nIVTT diagnostics mode, no output file generated\n\n");
  }
  ...
  while (igetl == 0) {
    ...
    if (igetl < 0) {  /* Normal EOF */
      ...
      if (mute == 0) {
        ...
        if (nldrpi > 0) {
          fprintf (stderr, "%7d for invalid locus\n", nldrpi);
        }
    ...
    /* Check page variables when new page read */
    if (newpage == 1) {
      pend_hd = 0;
      selpage = usepgloc(1);
      if (nlcopt || hastag) {
    ...
    /* Check page variables, text tags and locus types for all
