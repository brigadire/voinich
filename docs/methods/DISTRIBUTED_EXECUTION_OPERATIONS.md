...
```bash
go build -buildvcs=false -o conditional-regime-pki ./cmd/conditional-regime-pki
./cmd/conditional-regime-pki ca -out-dir pki
```
...
```bash
./cmd/conditional-regime-pki issue-coordinator \
...
```bash
./cmd/conditional-regime-pki issue-worker -ca-cert pki/ca.crt -ca-key pki/ca.key -worker-id worker-1 -out-dir pki
./cmd/conditional-regime-pki issue-worker -ca-cert pki/ca.crt -ca-key pki/ca.key -worker-id worker-2 -out-dir pki
```
...
```bash
./cmd/conditional-regime-analyze \
  -executor remote \
...
```bash
./cmd/structural-projection-analyze \
...
./cmd/replicated-local-structure-audit \
  -executor remote -workers 10 \
  -remote-listen 0.0.0.0:8443 \
  -tls-cert pki/coordinator.crt -tls-key pki/coordinator.key -client-ca pki/ca.crt \
  -remote-timeout 15m -remote-retries 3 \
...
./cmd/higher-order-sequence-validate \
  -executor remote -workers 10 \
  -remote-listen 0.0.0.0:8443 \
  -tls-cert pki/coordinator.crt -tls-key pki/coordinator.key -client-ca pki/ca.crt \
  -remote-timeout 15m -remote-retries 3 \
...
./cmd/positional-continuation-validate \
  -executor remote -workers 10 \
  -remote-listen 0.0.0.0:8443 \
  -tls-cert pki/coordinator.crt -tls-key pki/coordinator.key -client-ca pki/ca.crt \
  -remote-timeout 15m -remote-retries 3 \
...
./cmd/transition-network-validate \
  -executor remote -workers 10 \
  -remote-listen 0.0.0.0:8443 \
  -tls-cert pki/coordinator.crt -tls-key pki/coordinator.key -client-ca pki/ca.crt \
  -remote-timeout 15m -remote-retries 3 \
...
```bash
./cmd/conditional-regime-pki revoke -deny-list pki/deny.json -worker-id worker-3
./cmd/conditional-regime-analyze -executor remote ... -remote-deny-list pki/deny.json
```
...
`go build ./cmd/conditional-regime-analyze` once per unique GOOS/GOARCH pair
...
```bash
go build -o bin/conditional-regime-pki ./cmd/conditional-regime-pki
go build -o bin/pipeline-orchestrate ./pipelines/pipeline-orchestrate
