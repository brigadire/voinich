...
func (p *progressReporter) update(done, total int, label string) {
	...
	message := fmt.Sprintf("[%d/%d] %s [%s%s] %d/%d (%d%%) | elapsed %s", p.stage, p.stages, label, strings.Repeat("=", filled), strings.Repeat(".", width-filled), done, total, done*100/total, shortDuration(elapsed))
	if done > 0 && done < total {
		message += " | ETA " + shortDuration(eta)
	}
	p.line(message, final)
}
