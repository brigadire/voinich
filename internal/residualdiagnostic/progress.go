...
func (p *progressReporter) begin(stage int, label string) {
	...
	p.line(fmt.Sprintf("[%d/10] %s", stage, label), true)
}
...
func (p *progressReporter) update(done, total int, label string) {
	...
	msg := fmt.Sprintf("[%d/10] %s [%s%s] %d/%d (%d%%) | elapsed %s", p.stage, label, strings.Repeat("=", filled), strings.Repeat(".", 20-filled), done, total, done*100/total, shortDuration(elapsed))
	if done > 0 && done < total {
