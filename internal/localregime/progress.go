...
func (p *progressReporter) update(done, total int, label string) {
	...
	msg := fmt.Sprintf("[%d/%d] %s [%s] %d/%d (%d%%) | elapsed %s", p.stage, p.stages, label, strings.Repeat("=", filled)+strings.Repeat(".", w-filled), done, total, done*100/total, shortDuration(elapsed))
	if done > 0 && done < total {
